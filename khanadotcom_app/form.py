...
from .models import *  # Adjust import path as per your project structure
...
class OrderForm(forms.Form):
    ...
    def __init__(self, restaurant_id, *args, **kwargs):
        ...
        menu_items = MenuItem.objects.filter(restaurant_id=restaurant_id, availability=True)
        self.fields['items'].choices = [(item.menu_item_id, f"{item.name} - ${item.price}") for item in menu_items]